...
it("counts planned/attempted/completed separately and excludes missing timing and unscored results", async () => {
  let calls = 0;
  const report = await evaluateQuality(cases, ["a", "b"], 2, async provider => { calls++; return provider === "a" ? { status: "completed", text: "合成回答", model: "m", reasoning: "quick", durationMs: 100, firstTokenMs: 20 } : { status: "failed", text: "", error: "provider unavailable" }; });
