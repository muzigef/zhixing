...
it.each([1, 2, 3, 4, 5])("reads v%s without rewriting it, backs it up when saving v6, and rejects future versions", async (version) => {
  ...
  try {
    ...
    const loaded = await store.load(chat.id); expect(loaded.version).toBe(7);
    expect(await fs.readFile(file, "utf8")).toBe(original);
    await store.save(loaded); expect(await fs.readFile(`${file}.v${version}.bak`, "utf8")).toBe(original);
    expect(JSON.parse(await fs.readFile(file, "utf8")).version).toBe(7);
...
it("persists twelve model rounds and the exact project approval preview in v6", async () => {
  ...
  try {
    ...
    expect(restored.version).toBe(7); expect(restored.messages).toEqual(chat.messages);
  } finally { await fs.rm(root, { recursive: true, force: true }); }
});
