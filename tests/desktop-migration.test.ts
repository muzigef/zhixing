...
it.each([1, 2, 3, 4])("reads v%s without rewriting it, backs it up when saving v5, and rejects future versions", async (version) => {
  ...
  try {
    ...
    const loaded = await store.load(chat.id); expect(loaded.version).toBe(5);
    expect(await fs.readFile(file, "utf8")).toBe(original);
    await store.save(loaded); expect(await fs.readFile(`${file}.v${version}.bak`, "utf8")).toBe(original);
    expect(JSON.parse(await fs.readFile(file, "utf8")).version).toBe(5);
...
it("persists twelve model rounds and the exact project approval preview in v5", async () => {
  ...
  try {
    ...
    expect(restored.version).toBe(5); expect(restored.messages).toEqual(chat.messages);
  } finally { await fs.rm(root, { recursive: true, force: true }); }
});
...
it("marks the new database semantics so older binaries cannot silently discard product/permission evidence", () => {
  ...
  try { expect((db.db.prepare("SELECT MAX(version) AS version FROM schema_migrations").get() as { version: number }).version).toBe(5); }
  finally { db.close(); }
});
