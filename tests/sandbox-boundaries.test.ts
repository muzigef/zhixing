...
it("actually denies outside reads/writes, symlink escapes, network and subprocesses while allowing task files", async () => {
  ...
  const result = await execute(`const fs=require('node:fs'),net=require('node:net'),cp=require('node:child_process');
...
const child=cp.spawnSync(process.execPath,['-e','console.log("child")'],{timeout:500,windowsHide:true});
