import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { expect, it } from "vitest";
import { AgentService } from "../src/agent-service.js";
import { AgentSessionStore } from "../src/agent-session-store.js";
import { teamConfigurationSchema } from "../src/team-contracts.js";
import type { ModelClient } from "../src/model.js";
import { fixtureTeamReport, fixtureTeamReview } from "./team-fixtures.js";
import { randomUUID } from "node:crypto";
import { imageFromBytes } from "../src/image-input.js";
import { capabilitiesFor } from "../src/model-capabilities.js";

it("keeps single default and persists an actual team through the shared headless transport", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zhixing-team-service-")); let calls = 0;
  const client: ModelClient = { async *stream(_prompt, _signal, options) { calls++; const system = options?.messages?.filter(item => item.role === "system").map(item => item.content).join(" ") ?? ""; yield { type: "text_delta", text: system.includes("TEAM_PLAN") ? '{"tasks":["核查计算","检查条件"]}' : system.includes("TEAM_MEMBER") ? fixtureTeamReport : system.includes("TEAM_REVIEW") ? fixtureTeamReview : "计算结果是 4。" }; yield { type: "usage", usage: { inputTokens: 10, outputTokens: 20 } }; yield { type: "done" }; } };
  const service = new AgentService(new AgentSessionStore(root), () => client);
  try {
    const session = await service.create(); const request = { sessionId: session.id, provider: "mock" as const, style: "adaptive" as const, text: "2+2?" };
    await service.invoke(request); expect(calls).toBe(1); expect((await service.load(session.id)).messages.at(-1)?.team).toBeUndefined();
    const reply = await service.invoke({ ...request, collaboration: teamConfigurationSchema.parse({ mode: "same-model-team" }) });
    expect(calls).toBe(6); expect(reply.team?.status).toBe("completed");
    const message = (await new AgentSessionStore(root).load(session.id)).messages.at(-1)!;
    expect((await new AgentSessionStore(root).load(session.id)).version).toBe(11);
    expect(message.team?.tasks).toHaveLength(2); expect(message.team?.packet?.hash).toHaveLength(64);
    expect(JSON.parse(await fs.readFile(path.join(root, "conversations", `${session.id}.json.v8.bak`), "utf8")).version).toBe(8);
    expect(message.team?.members).toHaveLength(2); expect(message.collaboration?.mode).toBe("same-model-team");
    await expect(service.send({ ...request, resumeTaskId: message.taskId, retryTeamTaskId: message.team!.tasks![0]!.id })).rejects.toThrow("team_task_not_retryable");
    await expect(service.send({ ...request, retryTeamTaskId: randomUUID() })).rejects.toThrow("team_task_not_retryable");
    expect(calls).toBe(6);
    await expect(service.send({ ...request, resumeTaskId: message.taskId, collaboration: teamConfigurationSchema.parse({ mode: "single" }) })).rejects.toThrow("team_configuration_changed");
    const again = await service.invoke({ ...request, collaboration: message.collaboration });
    expect(calls).toBe(11); expect(again.team?.id).not.toBe(message.team?.id); expect(again.taskId).not.toBe(message.taskId);
    expect((await service.load(session.id)).messages.find(item => item.id === message.id)?.team?.id).toBe(message.team?.id);
  } finally { service.stop(); await service.idle(); await service.pauseMaintenance(); await fs.rm(root, { recursive: true, force: true }); }
});
it("restores original images on team continuation and refuses replacement task data", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zhixing-team-image-resume-"));
  const picture = imageFromBytes("synthetic.png", Uint8Array.from(Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jB9kAAAAASUVORK5CYII=", "base64")));
  let calls = 0;
  const base: ModelClient = { async *stream(_prompt, _signal, options) {
    calls++; const attached = options?.messages?.filter(item => item.images?.length).flatMap(item => item.images!) ?? [];
    expect(attached.length).toBeGreaterThan(0); for (const image of attached) expect(image).toEqual(picture);
    const system = options?.messages?.filter(item => item.role === "system").map(item => item.content).join(" ") ?? "";
    yield { type: "text_delta", text: system.includes("TEAM_PLAN") ? '{"tasks":["核查图片"]}' : system.includes("TEAM_MEMBER") ? fixtureTeamReport : system.includes("TEAM_REVIEW") ? fixtureTeamReview : "已核对图片。" };
    yield { type: "usage", usage: { inputTokens: 10, outputTokens: 10 } }; yield { type: "done" };
  } };
  const client: ModelClient = { ...base, capabilities: { ...capabilitiesFor(base), inputModalities: ["text", "image"] } };
  const service = new AgentService(new AgentSessionStore(root), () => client);
  try {
    const session = await service.create(); const request = { sessionId: session.id, provider: "mock" as const, style: "adaptive" as const, text: "核查图片" };
    const reply = await service.invoke({ ...request, images: [picture], collaboration: teamConfigurationSchema.parse({ mode: "same-model-team", members: [{ role: "reasoning-checker" }] }) });
    const before = calls; await service.invoke({ ...request, text: "继续", resumeTaskId: reply.taskId }); expect(calls).toBe(before + 1);
    await expect(service.send({ ...request, resumeTaskId: reply.taskId, images: [{ ...picture, name: "different.png" }] })).rejects.toThrow("team_scope_changed"); expect(calls).toBe(before + 1);
  } finally { service.stop(); await service.idle(); await service.pauseMaintenance(); await fs.rm(root, { recursive: true, force: true }); }
});
