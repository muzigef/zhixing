...
import type { OutcomeMode, OutcomePhase, OutcomeSubmission, OutcomeSummary, OutcomeView } from "../../src/outcome-contracts.js";
...
export function OutcomePanel({ topicId, disabled, onLesson }: { topicId: string; disabled: boolean; onLesson: (session: ChatSession) => void }) {
  ...
  return <section className="assessment-panel outcome-panel" aria-label="学习效果验证">
    ...
    {!active && <div className="outcome-actions"><label>学习方式<select aria-label="验证学习方式" value={mode} disabled={locked} onChange={event => setMode(event.target.value as OutcomeMode)}><option value="zhixing">知行引导教学</option><option value="direct">同模型直接聊天</option></select></label><button disabled={locked || !snapshot || !["agent-development", "rag"].includes(topicId)} onClick={() => void action({ type: "outcome-start", topicId, mode })}>开始学习验证</button></div>}
    ...
    {active && <div className="outcome-active">
      <strong>{active.title} · {modeName(active.mode)} · {phases[active.stage]}</strong>
