...
export function OutcomePanel({ topicId, disabled, onLesson }: { topicId: string; disabled: boolean; onLesson: (session: ChatSession) => void }) {
  ...
  return <section className="assessment-panel outcome-panel" aria-label="学习效果验证">
    ...
    {!active && ["agent-development", "rag"].includes(topicId) && <details><summary>参加已分组的教学研究</summary><p>仅导入负责人发给你的分组文件。使用没有该主题验证记录的个人工作区；学习方式按预先分配固定。负责人应先说明数据用途和退出方式。</p><label>研究分组文件<input type="file" accept=".json,application/json" aria-label="研究分组文件" disabled={locked} onChange={event => { const file = event.target.files?.[0]; event.target.value = ""; if (file) void (async () => { try { if (file.size > 12000) throw new Error("分组文件过大"); const ticket = studyTicketSchema.parse(JSON.parse(await file.text())); await action({ type: "outcome-enroll", topicId, ticket }); } catch { setError("无法读取分组文件，请核对文件和主题。"); } })(); }} /></label></details>}
    ...
    {active && <div className="outcome-active">
      ...
      {active.study && <p>研究代号 {active.study.participantCode} · 按预先分配参与；退出仍保留已发生的研究记录。</p>}
    ...
    {snapshot && snapshot.trials.length > 0 && <>
      ...
      <p>以下为描述性记录：普通验证由你选择方式，研究记录按导入的分组参与；研究主分析需与负责人冻结的分配清单核对。题卷难度尚未校准，不能据此认定因果效果或已掌握。演示、模型信息缺失或变化、借助帮助、重复参与不进入首次独立对照。</p>
...
function OutcomeForm({ trial, disabled, submit }: { trial: OutcomeView; disabled: boolean; submit: (submission: OutcomeSubmission) => Promise<void> }) {
  const [answers, setAnswers] = useState([-2, -2, -2]); const [explanation, setExplanation] = useState(""); const [transferExample, setTransferExample] = useState("");
  const [assistance, setAssistance] = useState<OutcomeSubmission["assistance"] | "">("");
  return <form onSubmit={event => { event.preventDefault(); if (assistance) void submit({ answers, explanation, ...(transferExample.trim() ? { transferExample } : {}), assistance }); }}>
    ...
    <label>用自己的话说明判断依据<textarea aria-label="学习验证解释" maxLength={2000} disabled={disabled} value={explanation} onChange={event => setExplanation(event.target.value)} /></label>
    <label>{trial.transferPrompt ?? "请举一个不同的新场景，并解释判断依据。"}<textarea aria-label="学习验证迁移例子" maxLength={2000} disabled={disabled} value={transferExample} onChange={event => setTransferExample(event.target.value)} /></label>
    ...
    <button className="primary" disabled={disabled || answers.includes(-2) || !explanation.trim() || !transferExample.trim() || !assistance}>保存{phases[trial.stage]}</button>
  </form>;
}
