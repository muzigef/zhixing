...
export function DiagnosticsPanel() {
  ...
  return <div className="setting-section"><h3>连接表现与版本 {data?.version}</h3>
    ...
    <details><summary>按模型、思考强度与传输方式查看</summary>{data?.performance.flatMap((provider) => provider.variants.map((item) => <div key={`${provider.provider}/${item.model}/${item.reasoning}/${item.transport}`}><p>{provider.provider} · {item.model} · {item.reasoning} · {item.transport}：{item.samples} 次回答，首字 {time(item.firstTokenP50)}。已报告 Token：输入 {item.inputTokens}，输出 {item.outputTokens}，缓存命中 {item.cacheReadTokens}。</p><p>已记录 {item.measuredTurns} 次模型请求。逐次 P50：SDK 准备 {time(item.startupP50)} · 请求至首事件 {time(item.firstEventP50)} · 请求至完成 {time(item.requestP50)} · 完成后收尾 {time(item.processTailP50)}。</p></div>))}<p>请求耗时包含认证、网络和远端处理；首事件不等于首个文字。auto 是自动传输策略，可能回退。未报告的阶段显示“暂无”，不估算用量或费用。</p></details>
