...
export function DiagnosticsPanel() {
  ...
  return <div className="setting-section"><h3>连接表现与版本 {data?.version}</h3>
    ...
    <details><summary>按模型与思考强度查看</summary>{data?.performance.flatMap((provider) => provider.variants.map((item) => <p key={`${provider.provider}/${item.model}/${item.reasoning}`}>{provider.provider} · {item.model} · {item.reasoning}：{item.samples} 次，首字 {time(item.firstTokenP50)}，Provider 准备 {time(item.startupP50)}。已报告 Token：输入 {item.inputTokens}，输出 {item.outputTokens}，缓存命中 {item.cacheReadTokens}。</p>))}<p>未报告用量的请求不估算费用。</p></details>
