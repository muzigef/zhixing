...
export function ApiConnectionsPanel({ profiles, settings, busy, onSave, onUpdated, onBusy }: {
  ...
  async function save() {
    ...
    if (!checked.success) { setError("请检查名称、API 根地址、模型 ID 和上下文预算。地址需使用 HTTPS（本机可用 HTTP），不要填写 /chat/completions 或查询参数。"); return; }
    await act(async () => {
  ...
  return <div className="custom-connections">
    ...
    <p className="connection-help">支持 OpenAI 兼容的 Chat Completions 接口。同一家可添加多个模型，不同服务的 Key 独立加密保存。</p>
    ...
    {editing && <form className="connection-form" onSubmit={event => { event.preventDefault(); void save(); }}>
      ...
      {original ? <p className="connection-help">可修改名称或替换 Key。更换地址、模型或兼容参数，请添加新连接，以保留旧任务的模型身份。</p> : <details className="connection-advanced"><summary>能力与兼容选项</summary>
        ...
        <label><input type="checkbox" checked={draft.streamUsage} onChange={event => setDraft({ ...draft, streamUsage: event.target.checked })} />请求流式 Token 用量（不兼容时关闭）</label>
        <label>思考参数<select aria-label="思考参数" value={draft.reasoning} onChange={event => setDraft({ ...draft, reasoning: event.target.value as ApiConnectionInput["reasoning"] })}><option value="none">不额外发送（默认）</option><option value="openai">OpenAI reasoning_effort</option><option value="deepseek">DeepSeek thinking</option><option value="kimi">Kimi reasoning_effort</option></select></label>
        <label>输出上限参数<select aria-label="输出上限参数" value={draft.tokenField} onChange={event => setDraft({ ...draft, tokenField: event.target.value as ApiConnectionInput["tokenField"] })}><option value="max_tokens">max_tokens（默认）</option><option value="max_completion_tokens">max_completion_tokens</option></select></label>
        ...
        <p>按服务商文档填写能力。只支持 Responses、Anthropic 原生 Messages 等其他协议的地址暂不能使用。</p>
      </details>}
