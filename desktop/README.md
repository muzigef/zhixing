...
当前版本的独立权限、项目快照/Python、任务核对、长历史、Skill 版本和完整产品效果验证见 [0.6 指南](../docs/agent-0.6.md)。交付验证见 [C01–C12 记录](../docs/evidence/agent-architecture-next.md)。

知行项目中的独立桌面对话包 `zhixing-desktop`，当前版本 `0.6.0`。提供连续学习对话、Pi Codex / DeepSeek 切换和本地会话管理；新增课程/资料/证据、排队/纠正、持久目标与耗时统计，见 [0.4 使用指南](../docs/agent-0.4.md)。完整项目介绍见 [根 README](../README.md)。
...
1. 打开本目录下的 `release/Zhixing-0.6.0-mac-arm64.dmg`。
...
该脚本生成 `desktop/release/mac-arm64/知行.app`、DMG 和 ZIP，当前版本对应 `Zhixing-0.6.0-mac-arm64.dmg` / `.zip`。上述本地命令不发布 Release 或安装到 Applications。没有签名证书时为预览包；明确构建未签名产物可设置 `CSC_IDENTITY_AUTO_DISCOVERY=false`。
