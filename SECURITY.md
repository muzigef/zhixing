...
- 桌面只实现文本对话，不给模型开放 Shell、文件工具或 CLI 学习工具。内附 Pi 显式使用空工具列表和已审查守卫；认证由 Pi 自己处理。
...
CLI 的引用校验验证文档与页码/锚点匹配，不保证逐句事实均有充分依据；桌面自由对话尚未接入资料引用校验。loopback 同步服务只提供本机 progress JSON/SSE，不是云同步。历史 Provider smoke 只证明当次请求结果，不证明当前登录持续有效。

桌面 macOS arm64 包目前为本地预览，没有 Developer ID 签名、公证或自动更新。Windows 只有打包配置，系统密钥存储和运行行为尚未实机验收。详见 [桌面证据](docs/evidence/desktop-app.md)。
