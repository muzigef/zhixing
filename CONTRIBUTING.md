...
5. 不提交用户资料、数据库、审计原文、API Key、token 或未经授权的真实 Provider 内容。受控合成评测可提交必要原答与脱敏结果，并注明合成范围、模型/预算、题集、包哈希及失败记录。

PR 描述以最终代码行为为准，附上复现方式与验证结果；按审查意见补齐测试和文档。当前没有仓库内定义的强制审查人数或自动合并规则。

CI 的 push/PR 工作流安装根目录和桌面两套依赖，执行两套生产依赖审计、完整 `verify` 和七组 Electron UI。手动/tag 发布工作流另做平台构建、实际包 UI、Windows NSIS 安装和校验和；正式签名需相应配置。请按[测试指南](docs/TESTING.md)完成适用验证并如实说明结果，不能以工作流存在或历史成功代替本次通过；当前远端状态见[当前状态](docs/current-status.md)。

新增兼容模型优先使用[通用配置与适配器契约](docs/provider-architecture.md)，不在 CLI 和桌面分别复制厂商逻辑。新增协议、原生运行时及团队策略须覆盖共享调用链、取消/恢复、能力限制、认证隔离和旧会话兼容；真实连接与模型质量评测单独记录。
