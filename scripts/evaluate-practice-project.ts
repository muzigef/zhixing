...
try {
  ...
  const service = new DesktopService(store, id => id === "pi-codex" ? pi : new DeepSeekClient(new MacOSKeychainSecretStore()), app);
  const session = await service.create();
  await service.send({ sessionId: session.id, provider: provider as "pi-codex" | "deepseek-api", topicId: "rag", contextAllowed: true, execution: "session", style: "adaptive", reasoning: "balanced", text: "这是临时合成项目的接入验收。请使用当前连接的项目工具，在 README.md 末尾添加一行“合成项目验证：已检查示例文件。”，保留原有内容。不要改其他文件。建立包含修改、实际测试、Git 检查点的项目计划，运行已有 .test.mjs，测试通过后保存本地 Git 检查点，再简短说明结果。无需查询学习资料或外部工具，不要只给出操作建议。" });
  await service.idle();
