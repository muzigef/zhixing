...
- [x] 当前 86 文件/419 测试、完整 verify、专项 8/8、四组 Electron UI；双真实 Provider 沿用上一轮合成记录，本轮未重复连接测试。

完成标准与过程见[冻结计划](docs/p0-agent-kernel-plan.md)、[开发验收](docs/evidence/p0-development-20260907.md)和[最新恢复边界核查](docs/evidence/p0-recovery-audit-20260907.md)。这三项是本次架构目标，不重新定义上面的历史 MVP P0；当前源码尚未重新发布安装包。
...
- [ ] 真实学习者 3 天复习、独立解释复核、题卷难度校准和效果对照；当前源码尚未重新打包。
...
P12 与 0.4.1 延迟修复的本机开发及打包验证已完成，见 [顺序执行计划](docs/agent-next-plan.md)、[验收证据](docs/evidence/agent-next.md)与[0.4 使用指南](docs/agent-0.4.md)。真实答案仍有续写重复、冗余提问和归因不精确等残余问题，逐题记录在[质量报告](docs/evidence/agent-quality-review.md)，不能以自动测试通过宣称商业 Agent 质量对齐。MCP/通用编码扩展和外部平台边界见 [Backlog](docs/implementation-backlog.md)。
