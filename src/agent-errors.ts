export function publicError(error: unknown): string {
  ...
  if (code === "native_subscription_required") return "请先在所选官方客户端完成订阅登录：Codex 使用 codex login，Claude Code 使用 claude auth login。知行不会读取或导入 token，也不会自动改用 API 计费。";
  if (code.startsWith("native_") || code === "team_native_budget_unsupported") return ({ native_model_required: "团队需要固定具体模型；请在官方 Agent 设置中指定模型，不能使用自动选择。", native_runtime_unavailable: "未找到可运行的官方 Agent，请安装官方客户端并完成登录。", native_runtime_failed: "官方 Agent 未完成，请在其客户端检查登录、订阅额度及网络。不会自动改用付费 API。", native_isolation_unavailable: "当前官方运行时尚不满足知行的隔离要求，请查看设置中的官方 Agent 状态。", native_cancel_unconfirmed: "已请求停止，但未确认官方进程退出。请检查该进程；远端用量状态未知。", native_agent_task_required: "官方 Agent 需要走共享会话任务入口，不能作为普通模型调用。", native_resume_unsupported: "此任务有待处理工具状态，请使用原 API 通道恢复。", team_native_budget_unsupported: "官方 Agent 尚不能保证团队的共享调用预算，请选择单 Agent，或使用 API 连接组成团队。", native_tool_denied: "官方 Agent 返回了超出本次上下文任务范围的工具调用，执行已终止。" } as Record<string, string>)[code] ?? "官方 Agent 未完成，本轮没有自动切换 API。";
