...
import { bubblewrapExecutable, posixSandboxHelper } from "./posix-sandbox.js";
import { windowsSandboxHelper } from "./windows-sandbox.js";
/** A preflight, not a substitute for the actual isolated execution result. */
export function executionSupport(platform: NodeJS.Platform = process.platform, sandboxPresent = platform === "win32" ? Boolean(windowsSandboxHelper()) : platform === "linux" ? Boolean(bubblewrapExecutable() && posixSandboxHelper()) : Boolean(existsSync("/usr/bin/sandbox-exec") && posixSandboxHelper())) {
  ...
  return { available, platform, verification: "runtime_result_required" as const,
    ...
    message: available ? `已找到 ${platform === "win32" ? "Windows AppContainer" : platform === "linux" ? "Linux bubblewrap" : "macOS 系统"}沙箱；测试是否成功仍以实际运行结果为准。` : !supported ? "本平台可使用对话与资料功能，实践代码执行暂无已验证的系统沙箱。" : "未找到系统沙箱，暂不能执行实践代码。" };
}
