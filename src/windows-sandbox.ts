    ...
    if (/^python(?:3(?:\.\d+)?)?\.exe$/i.test(path.basename(resolved))) {
      ...
      for (const name of ["Lib", "DLLs"]) {
        ...
        await fs.cp(source, path.join(runtime, name), { recursive: true, filter: async file => {
          options.signal?.throwIfAborted();
          // Generated bytecode is optional; copying thousands of host cache
          // files adds cold-start I/O. -B keeps the private stdlib read-only.
          if (["site-packages", "__pycache__"].includes(path.basename(file).toLowerCase())) return false;
          const stat = await fs.lstat(file); if (stat.isSymbolicLink()) return false;
