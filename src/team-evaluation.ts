...
interface RequestRecord { provider: AgentProvider; model?: string; phase: "planning" | "member" | "answer"; durationMs: number; maxOutputTokens?: number; usage?: ModelUsage; completed: boolean; }
...
function metered(client: ModelClient, provider: AgentProvider, records: RequestRecord[]): ModelClient {
  ...
  async function* request(prompt: string, signal: AbortSignal, options?: ModelRequestOptions, results?: readonly ToolResultMessage[]): AsyncIterable<ModelEvent> {
    ...
    const record: RequestRecord = { provider, model: client.identity?.model, phase: system.includes("TEAM_PLAN") ? "planning" : system.includes("TEAM_MEMBER") ? "member" : "answer", durationMs: 0, maxOutputTokens: options?.maxOutputTokens, completed: false };
    records.push(record); const started = Date.now();
    try {
      const events = results ? (client as ContinuableModelClient).continue(prompt, results, signal, options) : client.stream(prompt, signal, options);
      for await (const event of events) { if (event.type === "usage" && event.usage) record.usage = event.usage; if (event.type === "done") record.completed = true; yield event; }
...
export async function evaluateTeams(options: { root: string; suite: "pilot" | "holdout"; resolve: (provider: AgentProvider) => ModelClient; signal: AbortSignal; onProgress?: (progress: EvaluationProgress) => void }) {
  const startedAt = new Date().toISOString(); const cases = options.suite === "pilot" ? teamDevelopmentCases : teamHoldoutCases;
  const repeats = options.suite === "pilot" ? 1 : 2;
  ...
  const report = () => ({ version: 2, suite: options.suite, startedAt, finishedAt: new Date().toISOString(), planned: cases.length * repeats * evaluationArms.length,
    datasetHash: createHash("sha256").update(JSON.stringify(cases)).digest("hex"), bindings, unavailable: Object.fromEntries(unavailable), reasoning: "balanced", rows, summary: summarizeTeamEvaluation(rows), comparisons: comparisons(rows),
    limits: { maxModelRequestsPerArm: 12, rootDeadlineMs: 240_000, memberDeadlineMs: 90_000, maxOutputPerRequest: 4096, teamMaxOutputTokens: 16_384 },
  ...
  const run = async (item: TeamEvaluationCase, repeat: number, arm: EvaluationArm): Promise<EvaluationRow> => {
    ...
    const service = new AgentService(store, provider => { const client = clients.get(provider); if (!client) throw new Error("provider_not_found"); return metered(client, provider, requests); });
    ...
    try {
      for (let round = 0; round < (arm === "self-review-pi" ? 3 : 1); round++) {
  ...
  for (let repeat = 0; repeat < repeats; repeat++) for (const [index, item] of cases.entries()) {
    const offset = (index + repeat * 3) % evaluationArms.length;
    for (const arm of [...evaluationArms.slice(offset), ...evaluationArms.slice(0, offset)]) {
      options.onProgress?.({ completed: rows.length, total: cases.length * repeats * evaluationArms.length, caseId: item.id, arm, startedAt });
