...
export const observationSchema = z.object({ id: z.string().uuid(), topicId: topicIdSchema, source: z.object({ kind: z.literal("assessment"), dayId: dayIdSchema }), original: z.string().max(4000), assistance: assistanceSchema, correctCount: z.number().int().nonnegative(), total: z.number().int().positive(), errorCauses: z.array(z.string()).max(10), submittedAt: z.string().datetime(), revision: z.number().int().positive(), annotation: z.string().max(2000), withdrawn: z.boolean(), changes: z.array(revisionSchema).max(100) });
