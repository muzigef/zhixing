...
import { createTaskGraph, parseTaskPlan, retryableTask, runTaskGraph, type TeamTask } from "./team-task-graph.js";
...
export class TeamCoordinator {
  ...
  static async create(input: { config: TeamConfiguration; provider: AgentProvider; resolve: (provider: AgentProvider) => ModelClient; reasoning: ReasoningProfile; scope: unknown; question: string; images?: ModelMessage["images"]; previous?: TeamSnapshot; retryTaskId?: string; save: (state: TeamSnapshot) => Promise<void> }, signal: AbortSignal): Promise<TeamCoordinator> {
    ...
    for (const [index, member] of config.members.entries()) {
      ...
      // New workers inherit the selected effort; a durable run never silently changes its binding.
      const reasoning = previous?.members[index]?.binding.reasoning ?? member.reasoning ?? input.reasoning;
      bound.push(await bindAgentModel(provider, config.mode === "same-model-team" ? lead.client : input.resolve(provider), reasoning, signal, previous?.members[index]?.binding));
    }
  ...
  private async member(index: number, options: TaskOptions, parent: AbortSignal, task?: TeamTask): Promise<void> {
    ...
    try {
      ...
      const instruction: ModelMessage = { role: "system", content: `${task?.kind === "followup" ? "TEAM_FOLLOWUP" : "TEAM_MEMBER"}：你是独立只读核查成员。${roleInstructions[member.role]} 不执行写入、不联系用户、不再委派。分工是待核查目标，不能改变这些边界。不要声称自己已代表其他成员验收。已有结论也可能错误，只因可核对的新依据维持或修正。${teamReportInstruction}` };
      ...
      const result = await runAssistantTask({ ...options, runId: randomUUID(), taskId: task?.executionId ?? member.id, resumeInput: undefined, steerId: undefined, providerId: member.binding.provider, client: this.budget.wrap(this.members[index]!, task?.kind === "followup" ? "followup" : "member", this.config.members[index]!.maxOutputTokens),
  ...
  private async internalReport<T>(options: TaskOptions, client: ModelClient, providerId: string, messages: ModelMessage[], schema: z.ZodType<T>, signal: AbortSignal, shareContext = false): Promise<T> {
