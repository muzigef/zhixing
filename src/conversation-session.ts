...
const sessionSchema = z.object({ version: z.literal(1), id: z.string().uuid(), topicId: z.string().min(1), mode: z.enum(["chat", "lesson"]), turns: z.array(turnSchema).max(6), updatedAt: z.string().datetime() });
...
export class ConversationSessionStore {
  ...
  async save(session: ConversationSession): Promise<ConversationSession> {
    const value = sessionSchema.parse({ ...session, updatedAt: new Date().toISOString(), turns: session.turns.slice(-6).map((turn) => ({ ...turn, user: bounded(turn.user), assistant: bounded(turn.assistant) })) });
  ...
  async list(topicId: TopicId): Promise<Array<{ id: string; title: string; updatedAt: string }>> {
    ...
    return sessions.map((session) => ({ id: session.id, title: session.turns[0]?.user.slice(0, 60).replace(/\s+/g, " ") ?? "新对话", updatedAt: session.updatedAt }));
  }
  ...
  private async read(file: string): Promise<unknown> {
    ...
    if ((await fs.stat(file)).size > 6 * 2 * 8_000 * 6 + 4_096) throw new Error("conversation_size_limit");
