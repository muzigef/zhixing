...
export class DocumentLibrary {
  ...
  async importFile(topicId: TopicId, inputFile: string, signal?: AbortSignal): Promise<ImportResult> {
    ...
    if (existing) return { status: "duplicate", documentId: existing.id, chunks: 0 };
    ...
    await this.paths.assertNoSymlink(topicId, "library");
    await fs.copyFile(inputFile, destination);

    let pages: Array<{ text: string; page: number | null; anchor: string | null }>;
    try {
      if (signal?.aborted) throw new ImportRejectedError("cancelled");
      pages = ext === ".pdf" ? await this.extractPdf(source) : this.extractMarkdown(source.toString("utf8"));
      if (signal?.aborted) throw new ImportRejectedError("cancelled");
    } catch (error) {
      ...
      this.database.addDocument(documentId, topicId, sha256, name, ext === ".pdf" ? "application/pdf" : "text/markdown", status);
      return { status, documentId, chunks: 0, reason };
    }
    let lowConfidence = false;
    if (!pages.some((page) => page.text.trim()) && ext === ".pdf") {
      try {
        const ocrPages = await this.ocr.extract(destination);
        pages = ocrPages.map((page) => ({ text: page.text, page: page.page, anchor: null }));
        lowConfidence = ocrPages.some((page) => page.confidence < 70);
      } catch {
        this.database.addDocument(documentId, topicId, sha256, name, "application/pdf", "ocr_required");
    ...
    try {
      if (!this.database.addDocument(documentId, topicId, sha256, name, ext === ".pdf" ? "application/pdf" : "text/markdown", lowConfidence ? "ocr_low_confidence" : "indexed")) throw new Error("duplicate");
      let count = 0;
      for (const page of pages) for (const text of chunkText(page.text)) {
        if (signal?.aborted) throw new ImportRejectedError("cancelled");
        const chunkId = crypto.randomUUID();
        this.database.addChunk(chunkId, topicId, documentId, text, page.page, page.anchor, crypto.createHash("sha256").update(text).digest("hex"));
        this.database.addEmbedding(chunkId, topicId, this.embedding.embed(text));
        count += 1;
      }
      ...
      if (isAbort(error, signal) || error instanceof ImportRejectedError && error.message === "cancelled") {
        await fs.rm(path.dirname(destination), { recursive: true, force: true });
        this.database.addDocument(documentId, topicId, sha256, name, ext === ".pdf" ? "application/pdf" : "text/markdown", "rejected");
  ...
  private async extractPdf(source: Buffer): Promise<Array<{ text: string; page: number; anchor: null }>> {
    const pdfjs = await import("pdfjs-dist/legacy/build/pdf.mjs");
    // System fonts avoid browser-only file URL loading for standard PDF fonts in Node.
    const document = await pdfjs.getDocument({ data: new Uint8Array(source), useSystemFonts: true, verbosity: 0 }).promise;
...
function chunkText(text: string): string[] {
  ...
  for (const paragraph of paragraphs) {
    if (current.length + paragraph.length > 800 && current) { chunks.push(current); current = current.slice(-100); }
    current = `${current}${current ? "\n" : ""}${paragraph}`;
  }
  if (current) chunks.push(current.slice(0, 1000));
  return chunks;
}
