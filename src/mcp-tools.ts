  ...
  try {
    for (const server of configuration.servers.filter(server => server.enabled)) {
      ...
      try {
        ...
        base.harness.register({ name, input: z.object({}).strict(), risk: "read", idempotent: true, timeoutMs: 1000, execute: async (_value, context) => {
      ...
      for (const policy of server.tools) {
        ...
        const input = z.record(z.unknown()).refine(value => JSON.stringify(value).length <= 12_000);
        base.harness.register({ name, input, risk: policy.risk, idempotent: policy.risk === "read" && policy.replaySafe, timeoutMs: 18_000,
