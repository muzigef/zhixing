...
export function buildMessages(session: ChatSession, request: SendRequest): ModelMessage[] {
  ...
  if (session.study) {
    ...
    return [
      ...
      ...currentRules, { role: "user", content: request.text },
    ];
  }
  ...
  const corrections = session.messages.flatMap((message, index) => {
    ...
    return user?.role === "user" ? [excerpt(user.text, 4000)] : [];
  }).slice(-4);
  return [
    ...
    { role: "observation", content: JSON.stringify({ goal: session.context?.goal || context.goal, constraints: session.context?.notes, summary: session.context?.summary, ...(corrections.length ? { explicitUserCorrections: corrections } : {}), omittedMessages: context.omittedMessages }) },
