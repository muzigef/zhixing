...
export const assistantItemSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ ...identity, ...resolution, kind: z.literal("approval"), title: z.string().max(200), tool: z.string().regex(/^(?:save_artifact|run_experiment|mcp_[a-zA-Z0-9_-]{1,100}|project_[a-z_]{1,60})$/), input: z.record(z.string(), z.unknown()), preview: z.string().max(48_000).optional(), permissionLabel: z.string().max(500).optional() }),
