...
export const teamConfigurationSchema = z.object({
  ...
  members: z.array(z.object({ role: memberRoleSchema, provider: providerSchema.optional(), required: z.boolean().default(true) }).strict()).min(1).max(2).default([{ role: "reasoning-checker", required: true }, { role: "material-checker", required: true }]),
  ...
  memberTimeoutMs: z.number().int().min(5000).max(120_000).default(90_000),
...
export const teamMemberSnapshotSchema = z.object({
  ...
  attempts: z.number().int().min(0).max(3), durationMs: z.number().nonnegative().optional(),
...
export const teamStatusLabels: Record<TeamSnapshot["status"], string> = { preparing: "准备模型连接", planning: "主模型分工", running: "成员独立核查", merging: "主模型综合", completed: "已完成", partial: "部分完成", failed: "未完成", interrupted: "已中断", "not-needed": "本轮由本地流程完成" };
