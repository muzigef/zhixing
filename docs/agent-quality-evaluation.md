...
上述 `scripts/evaluate-agent.ts` 仍是固定 Pi / DeepSeek / demo 的旧回答质量运行器；产品支持更多连接不代表该脚本自动支持 Kimi、官方 Codex 或任意自定义 Provider。三模式对照使用 `src/team-evaluation.ts` 及桌面评测入口，当前报告v6、团队协议3、成员资源策略版本1，区分官方完整任务和API请求、实际档位、未知用量及首次完整消息时间。

2026-09-11 四题二十次真实对照见[预算结果](evidence/team-resource-budget-20260911.md)，后续通用接入重构只做[三家最小真实连接回归](evidence/provider-architecture-20260911.md)，没有重跑二十题次质量集。完成、答案字段正确、解释存在、解释语义正确和真实教学有效必须分别报告，不能互相替代。
