...
不加 `--live` 使用 demo；`--provider=pi-codex` 或 `--provider=deepseek-api` 可限定真实 Provider。每题两次独立会话、最多两个 Provider、十二题；使用临时合成资料，结束清理临时工作区。联网总开关为 0 时拒绝真实运行。

报告默认文件名带时间戳，明确输出路径已存在时拒绝覆盖。每次完成原子保存进度；Provider 首次无内容失败后，其后记录为未尝试，不复制耗时或用量。报告保存题目、种子历史、标准、答案、实际模型和思考档位、首字/总耗时、调用轮数、工具次数、用量与 worker 计时（仅 Provider 提供时）。数据集 SHA-256 绑定所选题目；代码哈希绑定本次加载的 `src/*.ts` 与 `desktop/core/*.ts`。代码哈希不替代发布构建签名。
