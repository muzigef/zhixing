...
核对日期：2026-09-18；当前存储与后端能力见下文。

桌面和 CLI 使用同一套记忆策略。请求通过入口校验后，给定相同工作区状态、完整会话、业务模式、请求、授权和模型配置，共享服务的输入组装、历史选择、摘要、工具回读与教学状态更新一致。CLI 与桌面的输入入口现均使用共享 20,000 字符上限，真实入口对照覆盖边界。模型随机采样的回答不要求逐字相同；不同工作区、独立会话或不同模型设置也不代表同一份输入。
...
| 关键原文 | `conversationAnchors` 单独保留显式目标、约束、纠正、未完成声明，最多 12 项、合计 6,000 字符，每项 700 字符，带消息 ID、行号和原文哈希；排除助手消息、引用行及代码围栏。省略项计数与截断标记可见。它只保留声明，不证明事实或自动解除冲突；当前纠正仍优先，必要时回读原文。受限教学实验继续使用其独立的预设上下文契约。 |
...
| CLI 兼容投影 | 最近六轮、每项 8,000 字符只用于旧格式、列表和兼容恢复；输入与模型历史不受该投影独立裁剪。CLI `execute`、共享请求契约和桌面均引用 20,000 字符上限；投影中的 8,000 字符不限制实际发送。 |

## 显式记忆更正

`search_memories` 返回当前主题的有效记录与原文哈希；`correct_memory` 绑定记录 ID、原文哈希和更正后内容，先展示更正预览并要求该项确认，普通会话写入授权不能代替。CLI 显式命令为 `更正记忆 <id> <内容> --确认`。数据库事务保留旧记录和来源，另建替代记录与更正关系；原文变化或另一更正已提交时拒绝竞争修改，相同操作重复提交返回同一结果。

被取代记录不再参与主题或全局召回；撤回替代记录不会自动恢复旧事实。没有明确更正关系的矛盾仍是待核实信息，不能只按新近性判断真假。这不是通用事实推理或知识图谱。
...
界面默认模型、设置/凭据存储和工作区位置可不同；解析成相同配置后才比较策略。基础会话写入 v8；团队相关字段按需升级至 v9–v13（成员资源策略需要 v13），升级前保留原文件副本，禁止降级覆盖；数据库版本为 9，包含显式记忆更正、PDF 逐页提取回执与切块版本/原文关联。升级不会自动合并两个入口的聊天目录。19,000 条起桌面提示保存容量，20,000 条或 12 MB 达上限时需新建会话。
...
2026-09-18 补充：连续摘要绑定 `summaryRecipe`、原文范围与摘要内容哈希，并记录生成配置身份；不匹配时保留原文但停止注入摘要。含此元数据或团队逐请求预算的会话写 v14，避免旧格式静默丢失语义。详见[派生产物依赖](derived-artifact-dependencies.md)和[预算核算](team-budget-accounting.md)。
