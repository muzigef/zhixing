...
当前源码为 `60570bc`、桌面包版本0.11.0；现行状态统一见 [当前状态](../current-status.md)。本地通用架构验收与最新远端CI结果分别记录：本地814项测试、七组实包UI及三家最小连接通过；远端同提交verify通过，但团队UI发送按钮超时，整体失败。最新候选包未在本轮替换已安装应用。
