...
最新故障复验：[原生安全存储与合并前 CI 复验](native-store-ci-20260918.md)，含启动等待循环、Ubuntu 环境、Windows 取消测试与三平台真实回执。此前故障复验：[Windows 沙箱 CI 修复](windows-sandbox-ci-20260917.md)，含目录竞态、Python 准备预算、四平台原生结果和失败历史。首次统一执行边界验收见[统一沙箱：2026-09-17](sandbox-20260917.md)。

应用架构基线为 `60570bc`、桌面包版本0.11.0，后续 CI 修复提交为 `485b358`；现行状态统一见 [当前状态](../current-status.md)。本地通用架构验收保留814项测试、七组实包UI及三家最小连接证据。远端 #44 团队UI超时已定位为测试等待竞态；修复提交的 [verify #46](https://github.com/muzigef/zhixing/actions/runs/34588292814) 已通过。最新候选包未在本轮替换已安装应用。
...
| 记录 | 范围与适用性 |
| --- | --- |
| [2026-09-18 原生存储 CI 与合并前复验](native-store-ci-20260918.md) | 1,023 项、七组 UI、远端 verify、四平台沙箱、三平台开发构建原生存储通过；签名安装包仍单独验收 |
| [2026-09-18 题库缺口开发](interview-improvements-20260918.md) | 持续开发记录，逐项验收；未完成内容见台账，不代表全部交付 |
| [2026-09-14 面试答案补全](interview-answers-20260914.json) | 60 题项目解答及追问答案、六个系统设计案例、源码依据与完整性核查；历史招聘和模型实验不作为新运行 |
| [2026-09-14 开发指令与设计说明](agent-docs-20260914.json) | 设计文档更名、两份文档与源码核对、引用迁移及本地回归；历史 UI 与模型结果独立保留 |
| [CI #44 团队 UI 竞态](ci-44-team-ui-20260911.md) | 按钮改名不等于保存结束；本机复现、受控保存闸门、实际弹窗关闭与输入回归 |
| [2026-09-11 文档核验](documentation-refresh-20260911.md) | 现行文档、历史适用范围、设计说明（现名 [agent-design.md](../agent-design.md)）、岗位调研与面试题库；当时发现的 CI 与入口差异单列 |
...
- [真实检索对照（2026-09-18）](retrieval-quality-20260918.json)：30 题公开开发集、真实 embeddinggemma 与词法/混合对比，逐题来源及失败口径。

- [分层 RAG 真实回执（2026-09-18）](rag-layered-accepted-20260918.json)：官方 Codex / 本机混合检索，见[完整失败与修复过程](interview-improvements-20260918.md)。

- [官方 Codex 四次性能探针（2026-09-18）](provider-performance-native-20260918.json)：固定短提示的真实任务耗时；两对样本不证明稳定提速。

- [本机原生安全存储失败回执](native-secure-store-macos-20260918.json)：2026-09-18，固定签名 Mac 实包初始化超时，真实加密/重启验收未通过。

- [Mac 实包 UI 回执](packaged-ui-macos-20260918.json)和[正式发行拒绝回执](formal-release-gate-macos-20260918.json)：七组 UI 通过，正式发布条件未齐，2026-09-18。
- [题库改进最终回执](interview-improvements-acceptance-20260918.json)：26 项工程、1,022 项测试与七组 UI；外部验收缺口单列，2026-09-18。
