...
1. 创建项目或导入副本。可选 JavaScript 或 Python 模板，分别内置 `.mjs` / `.py` 实现和测试。
...
模型使用 `project_status`、`project_read`、`project_edit`、`project_edit_many`、`project_patch`、`project_restore`、`project_test`、`project_checkpoint`。读取文件和列表支持分页；修改包含完整新文本和刚读到的文件哈希，拒绝覆盖期间发生的修改。项目计划使用 `project_file_saved`、`project_tests_passed`、`project_checkpoint_saved` 完成条件，并绑定项目 ID；结束时按真实文件和测试重新核验。
...
- 测试执行 `.test.mjs` 和 `test_*.py`（Python 标准库 unittest），使用内附 Node / 本机受验证 Python 与本地沙箱，合计最长 10 秒；不执行任意 Shell、npm 安装或网络请求。当前 LocalSandbox 经统一策略选择 macOS Seatbelt、Windows AppContainer 或 Linux bubblewrap；运行器、helper、必要系统能力或受验证 Python 缺失时明确返回不可用。Windows 辅助程序/运行时准备还有独立时限，10 秒是测试执行预算，不承诺端到端耗时始终小于10秒。未运行的测试不能记为通过。
