...
知行是本地优先的学习 Agent。它用通用 Agent 的执行、工具、记忆和恢复能力，服务于学习者的独立解释、实践应用与延迟复习。本文面向开发者和面试讲解，说明架构与设计取舍；仓库开发约束由 [AGENTS.md](../AGENTS.md) 管理，桌面聊天指令来自独立的 [runtime-AGENTS.md](../desktop/runtime-AGENTS.md)。本文是普通设计文档，不作为 Agent 自动加载的开发规则或产品提示。

核对日期：2026-09-18；当前工作分支起点：`c373c03`，改进完成与外部验收范围见[台账](interview-improvement-plan-20260918.md)，桌面包声明 0.11.0、CLI 根包声明 0.1.0。具体版本、已验证结果和待完成项统一见[当前状态](current-status.md)，使用导航见[文档索引](README.md)。

## 设计原则

1. **模型提出建议，程序执行并核实。** 模型生成答案、计划或工具请求；权限、输入校验、真实执行结果、完成条件和持久化由程序负责。
2. **入口共享策略。** 桌面与 CLI 都经过 AgentService；相同会话、主题、授权和模型配置应采用相同策略。宿主负责 UI、配置注入和存储位置，不复制一套教学或记忆算法；输入上限现已统一为 20,000 字符并有真实入口对照。
...
CLI 宿主与桌面均引用共享 20,000 字符上限；真实入口测试覆盖 8,001、20,000 和 20,001 字符，见[改进记录](evidence/interview-improvements-20260918.md)。
...
| 领域 | 当前实现 | 选择原因与边界 |
| --- | --- | --- |
| API 接入 | Chat Completions、Responses、Messages 三种协议；十家配置模板，也可添加自定义兼容连接 | 按协议复用适配器，减少新增厂商时的代码改动；模型能力仍需声明和测试，模板不等于真实账号验收 |
| 订阅接入 | 官方 Codex 有界文本执行及团队；Claude Code 有单 Agent 适配但真实账号未验收；Gemini 原生执行未实现 | 官方运行时管理认证，知行通过 AgentExecutor 调用；订阅不自动提供 API 额度，不能承诺各运行时拥有相同工具与恢复能力 |
| 默认选择 | CLI 初始角色路由为 mock；桌面新配置为 `pi-codex`；默认单 Agent，已有设置按其保存值使用 | 明确模型身份和调用方式，失败不静默切换到其他付费通道 |
| 团队协作 | 同模型与异模型团队共享调度、任务依赖、核查与预算；最多两名只读成员，不递归创建子团队 | 通过可追踪的分工和定向补做控制复杂性；是否提高质量必须与单 Agent、自检和等预算方案比较 |
| 记忆与持久化 | 两端共用完整原文、有限模型上下文、连续摘要与授权学习快照；新会话 v8，按所用字段升级至 v9–v14 | 保存与发送分离，限制输入成本；摘要绑定规则、原文范围及摘要哈希，失配停止使用；匹配也不证明概括正确；旧格式可读不代表新版可安全降级 |
| 安全与执行 | 知行工具通过 ToolHarness、权限和隔离执行；Pi worker 仅做模型生成；官方运行时使用独立适配器 | 文档约束和模型承诺不能代替代码执行边界；恢复时区分已完成、失败和副作用未知 |

代码依据：[API 配置](../src/api-connection-config.ts)、[原生目录](../src/native-runtime-catalog.ts)、[原生适配器注册](../src/native-runtime-adapters.ts)、[团队契约](../src/team-contracts.ts)、[会话存储](../src/agent-session-store.ts)。详细参数集中在各模块文档，避免将配置默认值散落为多份互相矛盾的契约。
...
详细契约见[架构](architecture.md)、[记忆设计](agent-memory.md)、[团队设计](agent-team-kernel.md)和[模型接入](provider-architecture.md)。
...
Node.js 版本要求为 `24.8.x`。项目有根目录与桌面两套依赖，以下命令均在仓库根目录执行：
...
2026-09-11 的历史实现基线 `485b358` 已通过 [GitHub verify #46](https://github.com/muzigef/zhixing/actions/runs/34588292814)：814 项测试、七组桌面 UI 及两套生产依赖审计。它修正了 #44 将保存按钮改名误判为弹窗关闭的测试竞态，见[修复记录](evidence/ci-44-team-ui-20260911.md)。这是 2026-09-11 的工程验证结果；当次文档更名不代表重新验收真实模型、教学效果或已安装 App；2026-09-18 的开发与验收另见上述台账。

2026-09-14 本次文档更新已按源码核对，并重新通过完整 verify（153 个测试文件、814 项测试）和全仓库 149 份 Markdown 本地链接/锚点检查；详见[核查记录](evidence/agent-docs-20260914.json)。

不能据此宣称已实现任意订阅通用登录、所有厂商账号可用、团队普遍提高正确率、云端高并发生产服务或已证明教学效果。正式分发签名/公证、安装状态和最新 CI 也必须分别查询。[面试资料](interview/README.md)提供基于这些边界的讲解与追问题。
