...
`AgentService` 仍是两端唯一任务入口；每个请求携带可选 `collaboration` 快照，缺省单 Agent。`bindAgentModel` 固定模型 ID、连接与思考档位；Pi 在任务开始后不重新采用变化的偏好。`TeamCoordinator` 使用一轮分工、最多两个独立只读核查、主 Agent 综合，内部复用 `runAssistantTask`，没有另一套教学或工具循环。

成员拥有独立任务 ID、模型 history、执行日志和 ToolHarness。工具策略在定义发现与执行两层限制，后续加载的 MCP 也不能恢复写入；成员不能 `ask_user` 或 `plan_task`。共享历史及资料由团队分享开关与原会话权限共同决定。成员结果作为不可信 observation 输入主流程，不赋予工具证据或验收权。

`TeamBudget` 在真实请求前原子保存预留，覆盖分工、成员、综合及修复轮次；保存失败时禁止继续请求。连接准备只通过受控适配器，不向协调器返回凭据。准备耗时计入整题，Pi 同连接按顺序启动成员，释放请求槽后再执行工具。取消/未知用量保留保守预留，不伪造零消耗。

团队真相保存于原子会话文件。普通会话仍 v8，含团队配置或记录的会话写为 v9，保留旧副本并阻止降级覆盖。重启保留已完成结果、把未知成员标为中断，不自动重复调用；恢复/迁移校验会话和权限范围。显式新任务重跑保留原任务。成员失败保持 partial，必需成员缺失会阻止整轮标为完成。详见[三模式指南](agent-teams.md)。

动态依赖图、自动多轮补查、自由通信与团队写入没有在首版开放。质量由[固定六组对照](agent-team-evaluation-protocol-20260909.md)实测，运行完成不等于质量认证。
