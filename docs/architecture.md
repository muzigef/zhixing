...
`AgentService` 仍是两端唯一任务入口；每个请求携带可选 `collaboration` 快照，缺省单 Agent。`bindAgentModel` 固定模型 ID、连接与思考档位；Pi 在任务开始后不重新采用变化的偏好。`TeamCoordinator` 使用结构化任务图、最多两名只读成员的独立任务循环、主模型逐项审查、必要时一次定向复核及重新审查、主 Agent 综合，内部复用 `runAssistantTask`，没有另一套教学或工具循环。
