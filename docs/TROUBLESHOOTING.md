...
| “当前 tutor 适配器不支持知行工具调用” | Pi Codex 和 DeepSeek 在两个入口都支持应用工具；`codex-cli` 兼容通道只支持文本。学习数据、项目及外部工具分别授权，写入还需审批。 |
...
- 旧应用报告数据版本过新：使用 0.6 或更新的兼容应用；不要手动降级 v5/数据库 5。回退使用升级前完整备份。
- Python 隔离不可用：当前只验证 macOS 的固定系统/Xcode/Homebrew Python 路径，使用标准库 unittest；应用不代装 Python 或依赖，也不会把未运行显示为通过。
...
- 实践测试不可用：平台预检只证明存在执行前提，Windows/缺少系统沙箱时禁用执行，不回退无隔离运行。
