...
`command` 必须是绝对路径；参数单独列在 `args`，不经过 shell。只连接自己信任的本机服务：省略 isolation 或选择 trusted 时，该进程以用户权限运行，可能访问文件或网络；read 工具标签不提供 OS 保障。知行使用临时工作目录和最小环境，不继承模型密钥、登录信息、`NODE_OPTIONS` 或用户主目录变量。当前不提供 MCP 密钥、OAuth 或 HTTP 配置；不要把凭据写入参数。
